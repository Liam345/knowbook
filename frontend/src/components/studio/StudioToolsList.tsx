...
export const StudioToolsList: React.FC<StudioToolsListProps> = ({
  ...
  /**
   * Check if category has any options
   */
  const categoryHasOptions = (category: GenerationCategory): boolean => {
    return generationOptions.some((opt) => opt.category === category);
  };
  ...
  const renderCategorySection = (category: GenerationCategory) => {
    if (!categoryHasOptions(category)) return null;
