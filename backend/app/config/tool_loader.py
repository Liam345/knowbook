...
class ToolLoader:
    ...
    - tools/prd_agent/ - Tools for PRD generation
    - etc.
    """
    ...
    def load_tools_for_agent(
        ...
        - client_tools: We execute them (custom tools)
        - termination_tools: Signal agent completion

        Server tools use a special format with "type" field.
        Client tools use standard format with "input_schema".

        Args:
            category: Tool category (subdirectory name, e.g., "prd_agent")

        Returns:
            Dict with 'server_tools', 'client_tools', and 'all_tools'
        """
        category_dir = self.tools_dir / category
        ...
        for tool_file in category_dir.glob("*.json"):
            ...
            # Server tools have a "type" field with server tool identifier
